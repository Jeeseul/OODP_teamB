	...
	public void getInfo() {
		...
		do {
			do {
				...
				}while(endTime >24 || endTime <1 || startTime > endTime);
				
				//false = 예약할 수 없다는 의미 
				if(meetingRooms[meetingRoomNum-1].checkReservation(month,day,startTime,endTime)==0) {
			...
			int result = meetingRooms[meetingRoomNum-1].addReserve(month, day, startTime, endTime);
